...
Spatially continuous data are increasingly required across ecology, agronomy,
geochemistry, mining and environmental science, but are rarely available
directly: they must be predicted from point samples by spatial predictive
methods \citep{li2021}. The de-facto reference implementation, \texttt{gstat}
\citep{pebesma2004}, is feature-complete but bound to the \textsf{R} runtime and
slow at scale; GSLIB \citep{deutsch1998} is fast Fortran with no modern
bindings, WebAssembly or Python interface. No geostatistics engine combines a
single-binary, dependency-light deployment with cross-platform reproducibility,
a unified 2-D/3-D core, Python and WebAssembly bindings, and numerical fidelity
to gstat.
...
Python module reproduces the CLI bit-for-bit.
...
single-band 2D-gridded-coverage raster (write), implemented in pure Rust via
bundled SQLite and a PNG encoder, i.e.\ without a GDAL dependency.
...
that the parallel Rust implementation avoids.
...
We predict rare-earth grades on $\sim$1100 mine-tailings deposits from non-rare-earth
